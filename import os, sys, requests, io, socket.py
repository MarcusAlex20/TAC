...
while True:
    ...
    try:
        ...
        sleep(300)
        sys.exit()
        # Change the asset class to crypto

        # Update the historical data client to use the CryptoHistoricalDataClient
        histo_client = CryptoHistoricalDataClient(A_KEY, A_SECRET)

        def get_historical_data(symbol, timeframe, start, end):
            for x in range(10):
                try:
                    client = CryptoHistoricalDataClient(A_KEY, A_SECRET)
